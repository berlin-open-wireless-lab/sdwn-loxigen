...
public class CircuitSignalID implements OFValueType<CircuitSignalID> {
    ...
    public static final CircuitSignalID NO_MASK = NONE;
    public static final CircuitSignalID FULL_MASK = new CircuitSignalID((byte) 0xff,
                                                                        (byte) 0xff,
                                                                        (short) 0xffff,
                                                                        (short) 0xffff);
    ...
    public void write6Bytes(ByteBuf bb) {
        bb.writeByte(gridType);
        bb.writeByte(channelSpacing);
        bb.writeShort(channelNumber);
        bb.writeShort(spectralWidth);
    }

    public static CircuitSignalID read6Bytes(ByteBuf bb) throws OFParseError {
        return new CircuitSignalID((byte) bb.readUnsignedByte(),
                                   (byte) bb.readUnsignedByte(),
                                   (short) bb.readUnsignedShort(),
                                   (short) bb.readUnsignedShort());
    }
